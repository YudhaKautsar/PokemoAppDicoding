...
class PokemonRepositoryImplTest {
    ...
    fun `getPokemonList should return list of pokemon from api`() = runTest {
        ...
        `when`(apiService.getPokemonList(100, 0)).thenReturn(response)
    ...
    fun `getPokemonList should return favorites from database when api fails`() = runTest {
        // Given
        `when`(apiService.getPokemonList(100, 0)).thenThrow(RuntimeException("Network Error"))
        val localFavorites = listOf(PokemonEntity("Pikachu", "url", "image"))
        `when`(pokemonDao.getAllFavorites()).thenReturn(flowOf(localFavorites))
    ...
    @Test
    fun `getFavoritePokemon should return list of favorite pokemon from dao`() = runTest {
        // Given
        val localFavorites = listOf(PokemonEntity("Pikachu", "url", "image"))
        `when`(pokemonDao.getAllFavorites()).thenReturn(flowOf(localFavorites))

        // When
        val results = repository.getFavoritePokemon().toList()
